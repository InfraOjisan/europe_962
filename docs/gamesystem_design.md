...
- **TurnEngine**（`src/engine/turnEngine.ts`、実装済み）：本書 1.1 の5フェイズを管理する有限状態機械（TurnFSM）の実体。
  ...
  通して処理する。年始フェイズで死亡した君主を検出してSuccessionEngineへ、戦闘解決フェイズで同一州の敵対軍を
  検出してCombatEngine→CausalityGuardへ、年末集計フェイズでWarCheckへ、それぞれ処理を委譲し、結果
  （占領による領有権移動、退却による軍の移動、捕虜の発生、後継者危機の帰結、大戦の発生）を GameState に反映する。
  外交・行動フェイズは、AIの意思決定ロジック（9章）が未実装のため現状フェイズを進めるだけのスタブ。
- **CombatEngine**：3章の演算を実装。入力＝両軍のArmy/Region/指揮官、出力＝BattleOutcome（結果種別＋損耗値＋
  捕縛した指揮官＋退却した側）。史実シナリオでの検証結果は3.6節を参照。
- **CausalityGuard**：3.6節の「因果律の保護」。TurnEngineがCombatEngineの出力を状態に反映する直前に通す。
- **EventEngine**：疫病・宗教などランダム/条件イベントの抽選と適用（未実装、9章）。
...
戦闘解決・継承・捕虜/人質・ターン進行の一次実装と、それらを結ぶ史実シナリオ検証
（`combatEngine.historical.test.ts`）までは完了した。以降は主に「AIやプレイヤー入力が
埋まっていない部分」と「数値バランスの継続チューニング」が残る。

- **外交フェイズ・行動フェイズの実装**：TurnEngineの `runDiplomacy`/`runAction` は
  現状スタブ。AI勢力の意思決定ロジック（宣戦・同盟・軍配置・後継指定/養子縁組・
  身代金交渉などのコマンド選択の優先順位）を実装する必要がある。
- **戦術洗練度テーブルの拡充**：現在は数種の doctrine のみ。gamesystem_europe.md が
  挙げる指揮官（リチャード1世、サラディン、ジャンヌ・ダルクなど）を個別の特性・
  ボーナスとして反映するかは未定。
- **同一州で複数の遭遇戦が同時発生するケース**：TurnEngineの戦闘解決フェイズは
  現状「州ごとに敵対ペアを1組だけ解決する」簡略実装。複数勢力が同時に鉢合わせる
  ケース、挟撃・奇襲の実際の判定ロジック（現状は真偽値を渡す口だけがある）は未実装。
- 傭兵隊長の契約・略奪の経済モデル詳細（身代金・登用は実装済み）
- 宗教・疫病イベントの発生条件とパラメータ影響range（EventEngine自体が未実装）
- 年末集計フェイズの税収・維持費計算（TurnEngineには未実装）
