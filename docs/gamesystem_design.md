...
- 版図外勢力（13章）の発生確率・被害倍率の継続チューニング、および辺境州（`Region.frontier`）の
  実データへの本格的な付与（14章のマップ設計を受けた `initialState.ts` の拡張）
- プレイヤーのゲームオーバー（6.2章）のUI実装（現状はエンジン関数のみ。選択肢の提示・
  傍観モードでの視点切り替え画面は未着手）
- 拡張シナリオデータ（イングランド・イベリア等）の追加と、11章のイベント年表の
  no-op項目の有効化、14章のマップ設計に基づく州データの本格拡充（現状7州のデモのみ）
- ローカルPoCハーネス（8.2章、`server/`）から本格的なWeb SPA（8.1章）への置き換え。
  現状は認証・永続化・複数セッション対応が無い単独プレイ限定の構成であり、外部公開や
  複数人での利用には別途これらの実装が必要。
...
移動先の評価は隣接州1手先までの近視眼的なロジックであり、複数州にまたがる進軍計画や
複数軍団の協調行動（意図的な挟撃の演出等）は今後の課題（12章）。
...
同じ思想で、マップの解像度もプレイのテンポを優先して粗く保つ。現状の初期データ
（`initialState.ts`）は7州のみのデモセットであり、本節はその先の本格的な拡張（12章の
今後の課題）に向けた設計指針を示す。

### 15.2 マクロリージョン一覧（設計指針、今後の `initialState.ts` 拡張の土台）

`RegionArchetype`（10.1章）と `frontier`（14章の版図外勢力の対象か）を各リージョンに
割り当てる際の指針。実データ化は12章の今後の課題として段階的に行う。

| 圏域 | 主なリージョン例 | 主なarchetype | frontier |
|---|---|---|---|
| ブリテン諸島 | イングランド、スコットランド、ウェールズ、アイルランド | coastal_trade / nordic | いいえ |
| イベリア半島 | カスティーリャ、アラゴン、ポルトガル、（イスラム勢力圏：グラナダ等） | mediterranean | いいえ（14章の例外：レコンキスタは通常のFaction間紛争として扱う） |
| フランス（西フランク系） | イル＝ド＝フランス、ブルゴーニュ、アキテーヌ、ブルターニュ、プロヴァンス | continental / mediterranean（南部） | いいえ |
| 神聖ローマ帝国圏 | ザクセン、バイエルン、シュヴァーベン、ボヘミア、オーストリア | continental / alpine（アルプス地方） | いいえ |
| イタリア半島 | 教皇領、ヴェネツィア、ミラノ（ロンバルディア）、ナポリ・シチリア | mediterranean / coastal_trade（ヴェネツィア等） | いいえ |
| スカンディナヴィア | デンマーク、ノルウェー、スウェーデン | nordic | いいえ |
| バルト海沿岸 | プロイセン、リヴォニア | nordic / coastal_trade | いいえ |
| ポーランド・リトアニア | ポーランド、リトアニア | continental / steppe_frontier（東部） | 東部のみ、はい |
| ハンガリー・バルカン半島 | ハンガリー、セルビア、ブルガリア、ワラキア・モルダヴィア | continental / steppe_frontier | はい（オスマン圧力の主戦場） |
| ルーシ諸公国 | ノヴゴロド、キエフ、モスクワ | steppe_frontier | はい（モンゴルの主要な侵攻先） |
| 東ローマ帝国（ビザンツ） | コンスタンティノープル、アナトリア、アプリア方面（初期データに実装済み） | mediterranean | はい（実装済み：`region_byzantium`） |

### 15.3 隣接関係の設計方針

`Region.adjacency` は対称関係（`validateGameState` で検証済み）。マクロリージョン単位の
隣接は、実際の地理的隣接関係をベースにしつつ、海路での隣接（例：ヴェネツィアとダルマチア
沿岸、イングランドとフランドル）もゲーム上意味があれば追加してよい（海洋を挟んだ遠征・
交易路の抽象化）。3.1章の地形補正（`TerrainModifier`）・7章の地図UIの見た目とも整合させる。

### 15.4 現在の実装状況との対応

初期データ（`initialState.ts`）の7州（ザクセン・バイエルン・シュヴァーベン・
イル＝ド＝フランス・ブルゴーニュ・教皇領・東ローマ）は、上表の
「神聖ローマ帝国圏」「フランス」「イタリア半島」「東ローマ帝国」の一部を最小構成で
実装したデモセットである。東ローマ（`region_byzantium`）にのみ `frontier: true` を
設定済みで、14章の版図外勢力イベントの動作確認に使える。表の残りの圏域
（ブリテン諸島・イベリア半島・スカンディナヴィア・バルト海沿岸・ポーランド・
リトアニア・ハンガリー・バルカン半島・ルーシ諸公国）の実データ化は12章の今後の課題。
