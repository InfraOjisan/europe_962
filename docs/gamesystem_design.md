...
- AIの外交・行動頻度のバランス調整：27勢力シナリオで650年通しシミュレーションを実行した
  ところ、大戦は誤爆しない一方で領土の変化がやや少なめ（例：数世紀に数件程度の占領）。
  「1ターン1手」の抽象化（13.2章）と大戦回避バイアスが効きすぎている可能性があり、
  史実シナリオでの検証を継続する。
...
ノードをクリックすると詳細パネルに人口・税基盤・地勢等を表示する。
