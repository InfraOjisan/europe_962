...
- **無主化**：claimantが1人もいない場合、当該勢力は消滅し（`collapseFaction`）、
  所有していた州は `Region.owner` が実体の無い勢力IDのまま「係争地」として残る。
  隣接する生存勢力は、戦争状態を問わずこの州へ進出できる（`buildActionOptions`）。
  実際の接収（無血に近い形での領有権移転）は `turnEngine.ts` の
  `resolveInterregnumAnnexation` が戦闘解決フェイズの末尾で行う（実装済み。当初は
  「戦闘解決エンジン側の責務」とだけ設計してあったが受け皿が無く、滅亡した州が
  誰にも占領されず残り続けるバグがあった——ユーザー報告により発見・修正、12章参照）。
...
- **世界の勢力集約ペースの検証（4.4章の死亡・出生仮実装を追加した後に発見、要優先対応）**：
  1000年シミュレーションを複数回実行したところ、後継者不在の断絶・内乱による分派勢力の
  乱立と淘汰・征服が積み重なり、終盤には生存勢力が数勢力（時には1勢力）にまで集約される
  ケースを確認した（大戦の誤発生自体は `GREAT_WAR_MIN_ALIVE_FACTIONS`、6.1章で対処済み）。
  無主化した州の接収処理（4.3章、下記）を実装した後の調査で、当初の想定より深刻な
  ペースであることが判明した：ある実行では西暦1160年ごろまでに27州中26州が「生存する
  隣接勢力がどこにも存在しない」状態（＝周辺一帯ごと無主化）に陥っていた。無主化した州の
  接収自体は正しく機能する（直接隣接する生存勢力がいれば、実際に無血で接収される）ため、
  これは接収ロジックの不備ではなく、淘汰そのものの進行が速すぎることを示している。
  婚姻AI実装による断絶ペースの緩和が本命の対策と見込むが、内乱時の分派勢力生成ルール
  （既存の今後の課題）や征服ペースとの複合的な検証も要する、優先度の高い継続課題。
- **無主化した州の接収（実装済み、ユーザー報告）**：`succession.ts` の `collapseFaction`
  （後継者不在による勢力消滅、4.3章）は「州の owner はそのまま残し、実際の無血占領の
  可否判定は戦闘解決エンジン側の責務とする」とコメントに書きつつ、その受け皿が
  未実装だった。滅亡した勢力が最後まで領有していた州は `Region.owner` が実体の無い
  勢力IDのまま永遠に残り、隣接する生存勢力の軍が（`buildActionOptions` の侵攻ループが
  「戦争状態」を要求するため）進出する手段が無く、誰にも占領されない空白州として
  マップ上に蓄積し続けていた。`buildActionOptions` に「無主化した隣接州へは戦争状態を
  問わず接収に向かえる」選択肢を追加し、`runBattleResolution` の末尾に
  `resolveInterregnumAnnexation`（その州に居合わせた生存勢力の軍のうち最も実効兵力の
  大きいものへ、無血で領有権を移す）を新設して対応した。
