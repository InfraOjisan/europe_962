...
src/
  ...
  engine/
    ...
    succession.ts          # 継承・養子縁組・後継者危機/内乱（係数は継続チューニング対象）
                            # ※死亡・出生は turnEngine.ts の仮実装（4.4章）が駆動する
    ...
    turnEngine.ts                    # 5フェイズのターン進行（TurnFSM）を実際に回すオーケストレーション層。
                                      # 外交・行動フェイズのAI接続、多重戦闘・奇襲・挟撃の判定、
                                      # 大国キャンペーンAI、神聖ローマ皇帝の継承・選挙（4.5章）、
                                      # 死亡・出生の仮実装（4.4章）もここに実装
  data/
    initialState.ts   # 962年開始時点の初期データ（27州・27勢力。神聖ローマ帝国は
                       # 選帝侯・諸侯レベルまで分割、ポーランド/ハンガリー/イタリア諸国/
                       # イベリア3王朝/イングランドを含む。設計書15章）
    historicalEvents.ts # 中学校社会科レベルの世界史年表イベント（1054〜1799年）
    offMapThreats.ts    # 版図外勢力の史実データ（マジャール人〜オスマン＝サファヴィー、
                         # イスラム政権のイベリア圧力、962〜1639/1492年）
...
server/
  ...
  public/        # 静的フロントエンド（プレーンHTML/CSS/JS、ビルド不要）
                 # design/ui-mockup/ の中世羊皮紙調デザインを踏襲したビューア。
                 # mapLayout.js の座標を基に27州をノード＋隣接線のネットワーク図として
                 # SVG描画する欧州地図（「スペインの道」を強調線で表示、設計書15章）・
                 # 勢力一覧・軍団一覧・年表ログ・大戦ゲージ・次の年へボタン・勢力選択モーダル
```
...
> ⚠️ Ollama等、キー認証をしないローカル／自己ホストのOpenAI互換エンドポイントを
> `AI_PROVIDER_ENDPOINT` に向ける場合も、`AI_PROVIDER_API_KEY` には空でない何らかの値
> （ダミーで構わない。例：`ollama`）を入れる必要がある。キーの有無判定が単純な空文字列
> チェックのため、空のままだと「未設定」と見なされ、エンドポイントを設定していても
> 常に点数判断へフォールバックしてしまう（詳細は `.env.example` のコメント参照）。
